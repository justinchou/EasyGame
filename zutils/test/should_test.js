...
describe('ZUtils Should', () => {
    describe('Should Test', () => {
        ...
        before(function () {
            if (ConfigMocha.debug > 1) Logger.info('连接符号帮助阅读没有实际意义:\nto be been is that which and has have with at of same but does\n');
        });

        describe("Simple Type", () => {
            if (ConfigMocha.debug > 1) Logger.info('基本语法: xx.should.to/not.[equal eql exist include include.members .have.property have.members]\n');
            ...
            it('#eql', function () {
                if (ConfigMocha.debug > 1) Logger.info('equal 比较两个基本类型, eql 深度比较两个复杂类型 eql === deep.equal\n');
        ...
        describe('Object', () => {
            ...
            it('#own own.property own.include 不与nested同用', function () {
                if (ConfigMocha.debug > 1) Logger.info(".property .include 表示自己的属性和继承的属性");
                if (ConfigMocha.debug > 1) Logger.info(".own.property .own.include 表示自己的属性, 不包含继承的属性");
